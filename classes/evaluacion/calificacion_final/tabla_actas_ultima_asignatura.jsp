...
<td colspan="9" align="center" height="30"> 
  ACTAS DE CALIFICACIONES DE EVALUACI&Oacute;N DE ÚLTIMA ASIGNATURA </td>
</tr>
...
consultas="SELECT grupos.grado, grupos.cve_grupo, grupos.cve_periodo, grupos.nombre, grupos.nombre as nom2, "
	...
	+"AND (grupos.cve_grupo IN (Select cve_grupo from calificacionez_alumno where (opcion_calif = 4) and (cve_carrera = "+ intCveCarrera+")) )"
...
while (rsx.next())
{
	...
	ca.revizion_zierre_kalifikazionez();				
					...
					*/
					...
					xyz.intCvePeriodo = intCveCarrera;              
					xyz.intOpcionCalif = 4;
...
<td> 
					<%
//					if (ca.adeuda == 0)
//out.println(intBanderaImprimeActa4 );
					if ((ca.adeuda == 0)&&(intBanderaImprimeActa4 == 1))
					{
						
					%>				 
					 <a href="javascript:FActaUltimaAsignatura(<%=grupo%>,<%=intCvePeriodo%>)" class="liga"><%=nom_grupo%></a>
					<%
					}
					else
					{
//						nom_grupo = nom_grupo + " *** ";						
					%>
					<a href="javascript:FDocentes_adeudo(<%=grupo%>,4,<%=intBanderaImprimeActa4%>)" class="liga"><%=nom_grupo%></a>	
...
<td width="36%"> <%=rsx.getString(6)+"  "+rsx.getString(7)+"  "+rsx.getString(8)%> </td>
 ...
 <% if((ent.intTieneEnvioDeActaPorDir>0)&&(ent.intTieneValidacionSE==0))
	{
 %>
	<input name="btnCancelarEnvio" type="button" id="btnCancelarEnvio" value="Cancelar Envío" onClick="cancelarEnvioDeActaPorDirector(<%=intCveCarrera%>,<%=grupo%>,<%=intCvePeriodo%>,4);" >
 <%
	}
 %>
 </td>
